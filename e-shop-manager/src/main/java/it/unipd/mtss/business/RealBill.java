...
public class RealBill implements Bill {
	...
	public double scontoProcessoreMenoCaro(List<EItem> itemsOrdered){
		...
		for (EItem item : itemsOrdered){
			if(item.getEItemType()==EItemType.PROCESSOR && item.getPrice()<prezzoMinore){
	...
	private double regaloMouseMenoCaro(List<EItem> itemsOrdered){
		double prezzoMinore = Double.POSITIVE_INFINITY;
		for (EItem item : itemsOrdered){
			if(item.getEItemType().toString().equals("MS") && item.getPrice()<prezzoMinore){
				prezzoMinore = item.getPrice();
			}
		}
		return prezzoMinore;
	...
	private double regaloMouseTastieraMenoCaro(List<EItem> itemsOrdered){
		double prezzoMinore = Double.POSITIVE_INFINITY;
		for (EItem item : itemsOrdered){
			if((item.getEItemType().toString().equals("MS") || item.getEItemType().toString().equals("KB")) && item.getPrice()<prezzoMinore){
				prezzoMinore = item.getPrice();
			}
		}
		return prezzoMinore;
	...
	private Boolean regaloPerMinorenne(User user){
		...
		long diff= Duration.between(user.getDataNascita().toInstant(), now.toInstant()).toDays()/365;
